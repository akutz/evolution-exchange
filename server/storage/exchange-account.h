...
struct _ExchangeAccountClass {
	...
	/* signals */
	void		(*connected)		(ExchangeAccount *account,
						 E2kContext *context);
	void		(*new_folder)		(ExchangeAccount *account,
						 EFolder *folder);
	void		(*removed_folder)	(ExchangeAccount *account,
						 EFolder *folder);
};
...
ExchangeAccount *
		exchange_account_new		(EAccountList *account_list,
						 EAccount *adata);
E2kContext *	exchange_account_get_context	(ExchangeAccount *account);
E2kGlobalCatalog
...
const gchar *	exchange_account_get_standard_uri
						(ExchangeAccount *account,
						 const gchar *item);
gchar *		exchange_account_get_sanitized_path
						(const gchar *uri);
gchar *		exchange_account_get_standard_uri_for
						(ExchangeAccount *account,
						 const gchar *home_uri,
						 const gchar *std_uri_prop);
gchar *		exchange_account_get_foreign_uri
						(ExchangeAccount *account,
...
E2kContext *	exchange_account_connect	(ExchangeAccount *account,
						 const gchar *pword,
						 ExchangeAccountResult *result);
EFolder *	exchange_account_get_folder	(ExchangeAccount *account,
						 const gchar *path_or_uri);
GPtrArray *	exchange_account_get_folders	(ExchangeAccount *account);

GPtrArray *	exchange_account_get_folder_tree
						(ExchangeAccount *account,
						 gchar *path);
ExchangeHierarchy *
...
void		exchange_account_rescan_tree	(ExchangeAccount *account);
gchar *		exchange_account_get_password	(ExchangeAccount *account);
ExchangeAccountResult
		exchange_account_set_password	(ExchangeAccount  *account,
						 gchar *old_password,
						 gchar *new_password);
void		exchange_account_forget_password
						(ExchangeAccount *account);

void		exchange_account_set_save_password
						(ExchangeAccount *account,
						 gboolean save_password);

gboolean	exchange_account_is_save_password
						(ExchangeAccount *account);
...
void		exchange_account_is_offline	(ExchangeAccount *account,
						 gint *mode);
void		exchange_account_is_offline_sync_set
						(ExchangeAccount *account,
						 gint *mode);
...
ExchangeAccountFolderResult
		exchange_account_discover_shared_folder
						(ExchangeAccount *account,
						 const gchar *user,
						 const gchar *folder_name,
						 EFolder **folder);
void		exchange_account_cancel_discover_shared_folder
						(ExchangeAccount *account,
						 const gchar *user,
						 const gchar *folder);
ExchangeAccountFolderResult
		exchange_account_remove_shared_folder
						(ExchangeAccount *account,
						 const gchar *path);
ExchangeAccountFolderResult
		exchange_account_add_favorite	(ExchangeAccount *account,
						 EFolder *folder);
ExchangeAccountFolderResult
		exchange_account_remove_favorite
						(ExchangeAccount *account,
						 EFolder *folder);
gboolean	exchange_account_is_favorite_folder
						(ExchangeAccount *account,
						 EFolder *folder);
gchar *		exchange_account_get_username	(ExchangeAccount *account);
gchar *		exchange_account_get_windows_domain
						(ExchangeAccount *account);
gchar *		exchange_account_get_email_id	(ExchangeAccount *account);
gint		exchange_account_get_quota_limit
						(ExchangeAccount *account);
gint		exchange_account_check_password_expiry
						(ExchangeAccount *account);

/* Folder Size methods */
void		exchange_account_folder_size_add
						(ExchangeAccount *account,
						 const gchar *folder_name,
						 gdouble size);
void		exchange_account_folder_size_remove
						(ExchangeAccount *account,
						 const gchar *folder_name);
void		exchange_account_folder_size_rename
						(ExchangeAccount *account,
						 const gchar *old_name,
						 const gchar *new_name);
GtkListStore *	exchange_account_folder_size_get_model
						(ExchangeAccount *account);
void		exchange_account_scan_foreign_hierarchy
						(ExchangeAccount *account,
