...
/* g_hash_table_foreach() callback to destroy a ECalBackendExchangeTasksObject */
static void
free_object (gpointer key, gpointer value, gpointer data)
{
        ECalBackendExchangeTasksObject *obj_data = value;
...
remove_recurrence_cb (gpointer key, gpointer value, gpointer data)
{
        ...
        ECalComponent *comp = value;
        ECalBackendExchangeTasks *cbext = data;
	...
	/* update the set of categories */
        ...
        e_cal_backend_unref_categories (E_CAL_BACKEND (cbext), categories);
        e_cal_component_free_categories_list (categories);
}
